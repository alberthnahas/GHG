...
The same footprints were convolved with the inventories. Anthropogenic enhancements change by {{F_ANTH_CHANGE_CO2}}% for CO₂ and {{F_ANTH_CHANGE_CH4}}% for CH₄, because the added sensitivity falls mostly over sea and sparsely emitting land. Fire contributions grow with the recovered influence: methane fire rises by {{F_FIRE_CH4_GAIN}}% to {{F_FIRE_CH4_NEW}} ppb, fire CO₂ to {{F_FIRE_CO2_NEW}} ppm, and fire CO to {{F_FIRE_CO_NEW}} ppb, which is {{F_FIRE_CO_PCT_GFS}}% of the observed {{OBS_CO}} ppb with no background and no other combustion source yet accounted for. Emission into the 1,000 m layer multiplies every contribution by about {{F_LAYER_RATIO}}, so an elevated fire injection does not relieve the over-closure; it deepens it.

{{F_SOURCE_TABLE}}

Under hourly ERA5 meteorology the five-day sensitivity is {{F_ERA5_RATIO}} times the GFS value and the footprint is placed in largely different cells ({{F_ERA5_SPATIAL}}% cell-level absolute difference). Its methane fire enhancement is {{F_FIRE_CH4_ERA5}} ppb and its fire CO {{F_FIRE_CO_ERA5}} ppb, {{F_FIRE_CO_PCT_ERA5}}% of the observation. Across the four inversion anchors and this case, ERA5 gives {{F_ERA5_RATIO_MIN}} to {{F_ERA5_RATIO_MAX}} times the GFS sensitivity with seed spread below 2%, a structural difference traceable to ERA5's shallower midday boundary layer at BKT and its hourly winds; which driver is closer to the atmosphere is not established by this comparison.

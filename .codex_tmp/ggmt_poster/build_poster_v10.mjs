...
async function main() {
  await fs.mkdir(`${TMP}/render-v10`, { recursive: true });
  ...
  const panels = [
    ...
    { title: "5. A single station tests methane inventories", figure: INVERSION_FIGURE, alt: "Methane footprint, posterior emission multipliers, and withheld-hour evaluation", caption: "Figure 7. Mean footprint, prior and posterior methane multipliers, and withheld-hour error.",
      text: "Five-day HYSPLIT-STILT footprints link 27 quality-screened BKT hours in September to October 2019 to regional methane sources. Inventory plus modeled background overpredicts the observations by about 80 ppb. A positive-emission Bayesian fit lowers the anthropogenic multipliers to 0.58 within 500 km and 0.51 beyond, but every 95% interval still includes the inventory value, and a background-only baseline predicts withheld hours as well as the inversion does.\n\nThe result is a test rather than a correction: one mountain receptor, quarter-degree meteorology without resolved convection, and a near field dominated by a few inventory cells leave regional emissions weakly determined. Transport-error tuning, larger particle ensembles, and independent boundary conditions are required before a station-based estimate can revise an inventory." },
  ...
  slide.speakerNotes.textFrame.setText([
    ...
    "Figure sources: poster variants of figures/f2_flask_validation.png, f6_diurnal.png, f12_nocturnal_flux.png, f16_growth_fire.png and f24_trends_gradients.png built by scripts/a78_poster_figure_variants.py (report suptitle removed, poster headline added; plots unchanged); outputs/poster/poster_inversion_original.png (scripts/a77_poster_inversion_figure.py from outputs/hysplit/inversion tables), figures/f24_trends_gradients.png.",
    "Result 5 numbers come from the published inversion (outputs/hysplit/inversion/tables). Regenerate with a77 --source revision and update the text after the September 2026 simulation revision completes.",
  ...
  await writeBlob(`${TMP}/render-v10/poster-preview.png`, preview);
  const layout = await slide.export({ format: "layout" });
  await fs.writeFile(`${TMP}/render-v10/poster.layout.json`, await layout.text());

...
const OUT = `${ROOT}/GGMT_2026_A0_GHG_Monitoring_Poster_v12.pptx`;
...
async function main() {
  await fs.mkdir(`${TMP}/render-v12`, { recursive: true });
  ...
  await addImage(slide, "coverage-figure", `${ROOT}/outputs/poster/poster_f1_coverage.png`, "Hourly data availability by station and species", x1, y, colW, 305);
  addCaption(slide, "coverage-caption", "Figure 1. Hourly CO₂, CH₄, and CO data availability at the five BMKG sites.", x1, y + 307, colW);
  addText(slide, "coverage-text", "Record length varies substantially across the network. Bukit Kototabang supplies the 24-year observational anchor, while Jambi and Kemayoran begin in late 2023. Bariri and Sorong provide intermediate-length records beginning in 2021. This unequal temporal support means that site comparisons must distinguish short process studies from climatological or trend evidence.\n\nTogether the sites cover the region's principal environments: a mountain background station at 865 m in West Sumatra, a lowland peat landscape in Jambi, the Jakarta megacity, a montane rainforest in Central Sulawesi, and a coastal city in Southwest Papua. All five measure CO₂, CH₄, and CO hourly, and Bukit Kototabang additionally hosts NOAA flask sampling, which ties the network to the WMO reference scales.", x2, y + 4, colW, 360, {
    fontSize: 25,
  });
  ...
  const panels = [
    { title: "1. Independent flasks validate the record", figure: `${ROOT}/outputs/poster/poster_f2_flask_validation.png`, alt: "Independent flask validation", caption: "Figure 3. Co-located flasks validate the continuous CO₂ and CH₄ record.",
      text: "Same-hour NOAA flasks agree with the continuous record to +0.31 ppm CO₂ at r = 0.77 and reproduce CH₄ at r = 0.98. The comparison itself exposed an archive time-base error that had masked this agreement (−8.50 ppm, r = 0.28), showing that an independent co-located reference both anchors the continuous observations to a traceable scale and guards their metadata.\n\nAgreement across both CO₂ and CH₄ provides confidence that differences subsequently observed among Indonesian and global sites represent atmospheric structure rather than an uncontrolled instrumental offset. The flask comparison therefore establishes the foundation for interpreting background gradients, seasonal cycles, and long-term changes across the network." },
  ...
  await writeBlob(`${TMP}/render-v12/poster-preview.png`, preview);
  const layout = await slide.export({ format: "layout" });
  await fs.writeFile(`${TMP}/render-v12/poster.layout.json`, await layout.text());
